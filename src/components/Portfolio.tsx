        ...
        <div className="grid gap-px bg-line md:grid-cols-2 xl:grid-cols-3">
          {portfolioItems.map((item, index) => (
            <article key={item.id} className="motion-card group flex h-full flex-col bg-porcelain px-0 py-8 md:px-6 md:first:pl-0 xl:[&:nth-child(3n+1)]:pl-0 xl:[&:nth-child(3n)]:pr-0">
              ...
              <div className="mt-5 flex items-start justify-between gap-5"><div><p className="editorial-label text-caramel">{String(index + 1).padStart(2, "0")} / {item.category}</p><h3 className="font-display mt-2 text-3xl font-medium">{item.title}</h3></div><FiArrowUpRight className="motion-card-arrow mt-1 shrink-0 text-xl text-coffee" aria-hidden="true" /></div>
