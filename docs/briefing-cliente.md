# Briefing do Cliente

Documento reservado para registrar informações reais confirmadas com o cliente.

## Dados Confirmados

- Nome da empresa: Aconche Decor
- Especialização: fábrica de camas e cabeceiras personalizadas
- WhatsApp: +55 (11) 96349-6728
- Telefone comercial: +55 (11) 99789-4132
- Instagram: @aconche_Decor
- E-mail: aconchedecorcontato@hotmail.com
- Região de atendimento: Grande São Paulo
- Horário de atendimento: das 9h às 17h

## Status da Fase 9

Os dados confirmados acima já foram inseridos no site, incluindo identidade,
contatos, região de atendimento, horário e SEO local.

Os serviços exibidos além de camas e cabeceiras permanecem provisórios até a
confirmação do cliente. Prazo, entrega, instalação, diferenciais e portfólio
real não devem ser publicados como informações confirmadas nesta etapa.

## Conteúdo Provisório

Até a confirmação dos dados finais, o site utilizará as seguintes informações
genéricas:

- Serviços: camas personalizadas, cabeceiras personalizadas, cabeceiras estofadas, camas box sob medida, recamiers, puffs e estofados sob medida.
- Prazo: definido após a avaliação de cada projeto.
- Entrega e instalação: combinadas durante o atendimento.
- Diferenciais: medidas personalizadas, escolha de acabamento, produção sob medida e atendimento pelo WhatsApp.
- Imagens: espaços reservados até o recebimento das fotos reais.
- WhatsApp principal mantido: +55 (11) 96349-6728.

## Referências Informadas

### Regina Urbana

URL: https://reginaurbana.com.br/

Referência enviada pelo cliente como inspiração de segmento e posicionamento.

Pontos observados na referência:

- Foco em camas, cabeceiras, cabeceiras personalizadas, recamiers e puffs.
- Organização por categorias de produtos.
- Chamada para escolha por ambiente.
- Uso de imagens grandes para valorizar produtos.
- Seção institucional/sobre.
- Portfólio por coleções ou categorias.
- Chamada para orçamento e contato por WhatsApp.

Como usar no projeto:

- Usar apenas como referência de mercado e estrutura de conteúdo.
- Reforçar o foco em camas e cabeceiras personalizadas.
- Valorizar categorias, portfólio e chamada para orçamento pelo WhatsApp.
- Manter estética premium, confortável e artesanal.

Não copiar:

- Textos.
- Layout.
- Imagens.
- Identidade visual.
- Promessas comerciais específicas.
- Informações não confirmadas pelo cliente, como tecidos, iluminação, endereço ou garantias.

Pendências para confirmar com o cliente:

- Se o cliente quer uma estrutura de categorias parecida.
- Quais categorias realmente oferece.
- Quais fotos reais estarão disponíveis.
- Prazo de entrega.
- Informações sobre entrega e instalação.
- Diferenciais reais que podem ser publicados.
